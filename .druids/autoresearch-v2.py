...
branch. Write and commit:

- research/dashboard.html -- self-contained summary for the human.
- research/observations.md -- accumulated knowledge.
- research/plan.md -- what you intend to do next and why.

Commit regularly. Each commit is a temporal snapshot. The git history \
IS the journal.
...
async def program(ctx, spec="", gpu_hours="4", nudge_minutes="10", **kwargs):
    ...
    async def create_assistant():
        ...
        async def on_result(
            ...
            run = {
                "val_bpb": bpb, "peak_vram_mb": vram, "config": config,
                "keep": is_keep, "crashed": is_crash, "notes": notes,
                "time": time.time(),
            }
            ...
            if budget_remaining() <= 0:
                ...
                await scientist.send(
                    ...
                    "update the dashboard, and commit."
                )
    ...
    def register_scientist_tools(sci):
        ...
        async def on_spawn(reason: str = ""):
            ...
            return (
                f"Assistant {assistant_count - 1} spawned. "
                f"{idle_assistants.qsize()} idle, {assistant_count} total."
            )
        ...
        async def on_compact(summary: str = ""):
            ...
            You will be restarted with fresh context. Read your committed
            files on research/best to resume where you left off."""
